...
is a key task for cluster administrators. The  has some controls available
...
Some requests classified as long-runningmdashsuch as remote
command execution or log tailingmdashare not subject to the API
...
# # EnablingDisabling API Priority and Fairness

The API Priority and Fairness feature is controlled by a command-line flag
and is enabled by default. See
[Options](docsreferencecommand-line-tools-referencekube-apiserver#options)
...
```shell
kube-apiserver
--runtime-configflowcontrol.apiserver.k8s.iov1beta3false
 # and other flags as usual
```

The command-line flag `--enable-priority-and-fairnessfalse` will disable the
...
deep in the recursion, there is a danger of priority inversions andor
deadlocks.
...
moment when comparing against a priority levels concurrency limit. In
...
object createupdatedelete. To account for this work, API Priority
...
requests number of seats and seat occupancy time to include this
extra work.
...
is either the requesting user, the target resources namespace, or nothing  and the
...
a technique known as , which makes relatively efficient use of
...
[PriorityLevelConfigurations](docsreferencegeneratedkubernetes-api#prioritylevelconfiguration-v1-flowcontrol-apiserver-k8s-io)
define the available priority levels, the share of the available concurrency
budget that each can handle, and allow for fine-tuning queuing behavior.
[FlowSchemas](docsreferencegeneratedkubernetes-api#flowschema-v1-flowcontrol-apiserver-k8s-io)
...
nominalassured limits were always applied without adjustment.
...
percentages of the levels nominal limit. These are resolved to
...
[enhancement proposal](httpsgithub.comkubernetesenhancementstreemasterkepssig-api-machinery1040-priority-and-fairness), but in short
...
httpsplay.golang.orgpGi0PLgVHiUg , which computes this table.
...
smaller `name` will win, but its better not to rely on this, and instead to
...
A FlowSchemas `distinguisherMethod.type` determines how requests matching that
...
* The mandatory `exempt` priority level is used for requests that are
  ...
  requests from the `systemmasters` group into this priority
...
reasonable default configuration. You can modify these andor create
...
* The `system` priority level is for non-health requests from the
  `systemnodes` group, i.e. Kubelets, which must be able to contact
...
* The `leader-election` priority level is for leader election requests from
  ...
  or `leases` coming from the `systemkube-controller-manager` or
  `systemkube-scheduler` users and service accounts in the `kube-system`
...
inconsistent with the servers guardrail behavior.
...
it --- with the servers suggested spec --- if the object does not
...
object. In the former case, the server ensures that the objects spec
...
for an annotation with key `apf.kubernetes.ioautoupdate-spec`. If
...
object should set its `apf.kubernetes.ioautoupdate-spec` annotation
to `false`.

Maintenance of a mandatory or suggested configuration object also
includes ensuring that it has an `apf.kubernetes.ioautoupdate-spec`
...
`apf.kubernetes.ioautoupdate-spectrue`.
...
mechanism to protect your clusters API server from general internet
traffic, you can configure rules to block any health check requests
that originate from outside your cluster.

 code_sample filepriority-and-fairnesshealth-for-strangers.yaml
...
respectively. If youre running Kubernetes versions v1.19 and earlier, you
...
* `apiserver_flowcontrol_nominal_limit_seats` is a gauge vector
  holding each priority levels nominal concurrency limit, computed
  from the API servers total concurrency limit and the priority
  levels configured nominal concurrency shares.
...
* `apiserver_current_inqueue_requests` is a gauge vector of recent
  ...
  last windows high water mark of number of requests actively being
  served.
...
* `apiserver_flowcontrol_priority_level_seat_utilization` is a
  histogram vector of observations, made at the end of each
  nanosecond, of the utilization of a priority levels concurrency
...
* `apiserver_flowcontrol_request_queue_length_after_enqueue` is a
  ...
  if one priority levels histogram shows that all queues for that priority
  level are longer than those for other priority levels, it may be appropriate
  to increase that PriorityLevelConfigurations concurrency shares.
...
* `apiserver_flowcontrol_lower_limit_seats` is a gauge vector holding
  the lower bound on each priority levels dynamic concurrency limit.

* `apiserver_flowcontrol_upper_limit_seats` is a gauge vector holding
  the upper bound on each priority levels dynamic concurrency limit.
...
* `apiserver_flowcontrol_demand_seats` is a histogram vector counting
  ...
  levels ratio of (seat demand)  (nominal concurrency limit).
  A priority levels seat demand is the sum, over both queued requests
  and those in the initial phase of execution, of the maximum of the
  number of seats occupied in the requests initial and final
  execution phases.
...
* `apiserver_flowcontrol_epoch_advance_total` is a counter vector of
  the number of attempts to jump a priority levels progress meter
...
1. You can create a new FlowSchema which references a PriorityLevelConfiguration
   ...
   `apf.kubernetes.ioautoupdate-spec` annotation to false.
...
Example FlowSchema object to isolate list event requests
 ...
 code_sample filepriority-and-fairnesslist-events-default-service-account.yaml
...
- You can visit flow control [reference doc](docsreferencedebug-clusterflow-control) to learn more about troubleshooting.
- For background information on design details for API priority and fairness, see
  the [enhancement proposal](httpsgithub.comkubernetesenhancementstreemasterkepssig-api-machinery1040-priority-and-fairness).
- You can make suggestions and feature requests via [SIG API Machinery](httpsgithub.comkubernetescommunitytreemastersig-api-machinery)
  or the features [slack channel](httpskubernetes.slack.commessagesapi-priority-and-fairness).
