...
Resource Managers. The managers aim to co-ordinate and optimise the alignment of nodes resources for pods
...
resource (for example, assigning one of more CPUs for a Pods exclusive use).
...
- `none` the `none` policy explicitly enables the existing default CPU
  ...
  [Guaranteed pods](docsconceptsworkloadspodspod-qos) and
  [Burstable pods](docsconceptsworkloadspodspod-qos)
...
- `static` the `static` policy allows containers in `Guaranteed` pods with integer CPU
  ...
  using the [cpuset cgroup controller](httpswww.kernel.orgdocDocumentationcgroup-v2.txt).
...
CPU Manager doesnt support offlining and onlining of CPUs at runtime.
...
And the containers resource limit for the CPU resource is an integer greater than
...
The pod above runs in the `Guaranteed` QoS class because `requests` are equal to `limits`.
But the containers resource limit for the CPU resource is a fraction. It runs in
the shared pool.
...
containers resource limit for the CPU resource is an integer greater than or
...
to the [noisy neighbours problem](httpsen.wikipedia.orgwikiCloud_computing_issues#Performance_interference_and_noisy_neighbors).
...
The `reservedSystemCPUs` parameter in [KubeletConfiguration](docsreferenceconfig-apikubelet-config.v1beta1),
...
[Explicitly Reserved CPU List](docstasksadminister-clusterreserve-compute-resources#explicitly-reserved-cpu-list) page.
...
affine as many CPUs as possible within the same uncore cache. If the containers
...
- [Device Manager](docsconceptsextend-kubernetescompute-storage-netdevice-plugins#device-plugin-integration-with-the-topology-manager)
