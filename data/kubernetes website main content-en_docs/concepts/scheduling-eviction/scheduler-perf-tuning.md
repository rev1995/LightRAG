...
[kube-scheduler](docsconceptsscheduling-evictionkube-scheduler#kube-scheduler)
...
In large clusters, you can tune the schedulers behaviour balancing
...
[kube-scheduler configuration file](docsreferenceconfig-apikube-scheduler-config.v1)
and then restart the scheduler.
In many cases, the configuration file can be found at `etckubernetesconfigkube-scheduler.yaml`.
...
[scoring phase](docsconceptsscheduling-evictionkube-scheduler#kube-scheduler-implementation).
...
If you dont specify a threshold, Kubernetes calculates a figure using a
...
```yaml
apiVersion kubescheduler.config.k8s.iov1alpha1
...
schedulers performance significantly.
...
percentage to anything below 10, unless the schedulers throughput is critical
...
* Check the [kube-scheduler configuration reference (v1)](docsreferenceconfig-apikube-scheduler-config.v1)
