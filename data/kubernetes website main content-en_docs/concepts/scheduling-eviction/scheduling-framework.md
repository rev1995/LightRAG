...
[kep] httpsgithub.comkubernetesenhancementsblobmasterkepssig-scheduling624-scheduling-frameworkREADME.md
...
[Scheduler Configuration](docsreferenceschedulingconfig#extension-points).
...
Otherwise, its placed in the internal unschedulable Pods list, and doesnt get an `Unschedulable` condition.

For more details about how internal scheduler queues work, read
[Scheduling queue in kube-scheduler](httpsgithub.comkubernetescommunityblobf03b6d5692bd979f07dd472e7b6836b2dad0fd9bcontributorsdevelsig-schedulingscheduler_queues.md).
...
Its executed every time a certain kind of event or change happens in the cluster.
...
In Kubernetes , this feature gate is enabled by default,
and you can disable it via the
`SchedulerQueueingHints` [feature gate](docsreferencecommand-line-tools-referencefeature-gates).
...
This feature is applicable to CSI volumes that supported [Storage Capacity](docsconceptsstoragestorage-capacity),
...
func NormalizeScores(scores map[string]int)
    ...
    for node, score  range scores
        scores[node]  score*NodeScoreMaxhighest
...
(see [`FrameworkHandle`](httpsgit.k8s.ioenhancementskepssig-scheduling624-scheduling-framework#frameworkhandle)),
...
[plugins](docsreferenceschedulingconfig#scheduling-plugins) are in use and
enabled by default.
...
[scheduler-plugins](httpsgithub.comkubernetes-sigsscheduler-plugins) for more details.

If you are using Kubernetes v1.18 or later, you can configure a set of plugins as
a scheduler profile and then define multiple profiles to fit various kinds of workload.
Learn more at [multiple profiles](docsreferenceschedulingconfig#multiple-profiles).
