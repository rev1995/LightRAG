...
[Encrypt Secret Data at Rest](docstasksadminister-clusterencrypt-data).
...
[RBAC good practices](docsconceptssecurityrbac-good-practices).
...
- **Components** Restrict `watch` or `list` access to only the most
  ...
  the components normal behavior requires it.
...
If there are multiple `etcd` instances, configure encrypted SSLTLS
...
The [Kubernetes Secrets Store CSI Driver](httpssecrets-store-csi-driver.sigs.k8s.io)
...
[Providers for the Secret Store CSI Driver](httpssecrets-store-csi-driver.sigs.k8s.ioconcepts.html#provider-for-the-secrets-store-csi-driver).
