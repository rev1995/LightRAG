...
* Each [pod](docsconceptsworkloadspods) in a cluster gets its
...
* The _pod network_ (also called a cluster network) handles communication
  ...
  * All pods can communicate with all other pods, whether they are
    on the same [node](docsconceptsarchitecturenodes) or on
...
* The [Service](docsconceptsservices-networkingservice) API
  ...
  * Kubernetes automatically manages
    [EndpointSlice](docsconceptsservices-networkingendpoint-slices)
...
* The [Gateway](docsconceptsservices-networkinggateway) API
  (or its predecessor, [Ingress](docsconceptsservices-networkingingress))
  ...
  * A simpler, but less-configurable, mechanism for cluster
    ingress is available via the Service APIs
    [`type LoadBalancer`](docsconceptsservices-networkingservice#loadbalancer),
    when using a supported .

* [NetworkPolicy](docsconceptsservices-networkingnetwork-policies) is a built-in
...
hosts. This is not needed in Kubernetes Kubernetess model is that
...
* Pod network namespace setup is handled by system-level software implementing the
  [Container Runtime Interface](docsconceptsarchitecturecri.md).

* The pod network itself is managed by a
  [pod network implementation](docsconceptscluster-administrationaddons#networking-and-network-policy).
...
* NetworkPolicy is generally also implemented by the pod network
  implementation. (Some simpler pod network implementations dont
...
* There are many [implementations of the Gateway API](httpsgateway-api.sigs.k8s.ioimplementations),
...
The [Connecting Applications with Services](docstutorialsservicesconnect-applications-service)
tutorial lets you learn about Services and Kubernetes networking with a hands-on example.

[Cluster Networking](docsconceptscluster-administrationnetworking) explains how to set
