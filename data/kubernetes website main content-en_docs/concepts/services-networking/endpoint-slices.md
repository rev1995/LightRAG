...
for any Kubernetes Service that has a  specified. These EndpointSlices include
...
As an example, heres a sample EndpointSlice object, thats owned by the `example`
Kubernetes Service.
...
metadata
  name example-abc
  labels
    kubernetes.ioservice-name example
addressType IPv4
ports
  - name http
    protocol TCP
...
maps to the Pods `Ready` condition.
...
timestamp, but most likely before the Pods containers exit).
...
`endpointslice.kubernetes.iomanaged-by`, which indicates the entity managing
an EndpointSlice.
...
reference on each EndpointSlice as well as a `kubernetes.ioservice-name`
...
3. If theres still new endpoints left to add, try to fit them into a previously
   unchanged slice andor create new ones.
...
expect
to route traffic based on Endpoints resources, the clusters control
...
* the Endpoints resource has a `endpointslice.kubernetes.ioskip-mirror` label
  set to `true`.
* the Endpoints resource has a `control-plane.alpha.kubernetes.ioleader`
  annotation.
...
* Read the [API reference](docsreferencekubernetes-apiservice-resourcesendpoint-slice-v1) for the EndpointSlice API
* Read the [API reference](docsreferencekubernetes-apiservice-resourcesendpoints-v1) for the Endpoints API
