...
zone it originated from. When calculating the endpoints for a , the EndpointSlice controller considers
the topology (region and zone) of each endpoint and populates the hints field to
allocate it to a zone. Cluster components such as  can then consume those hints, and use
...
`service.kubernetes.iotopology-aware-hints` annotation.

You can enable Topology Aware Routing for a Service by setting the
`service.kubernetes.iotopology-mode` annotation to `Auto`. When there are
...
[allocatable](docstasksadminister-clusterreserve-compute-resources#node-allocatable)
...
metadata
  name example-hints
  labels
    kubernetes.ioservice-name example-svc
addressType IPv4
ports
  - name http
    protocol TCP
...
safeguard rules before using Topology Aware Hints. If these dont check out,
...
2. **Impossible to achieve balanced allocation** In some cases, it will be
   ...
   as zone-b. The controller does not assign hints if it cant get this expected
...
3. **One or more Nodes has insufficient information** If any node does not have
   a `topology.kubernetes.iozone` label or is not reporting a value for
...
* The EndpointSlice controller ignores nodes with the
  `node-role.kubernetes.iocontrol-plane` or `node-role.kubernetes.iomaster`
  label set. This could be problematic if workloads are also running on those
  nodes.

* The EndpointSlice controller does not take into account  when deploying or calculating the
...
* This may not work well with autoscaling. For example, if a lot of traffic is
  ...
  be handling that traffic. That could result in
...
* Learn about the
  ...
  field, which is closely related to the `service.kubernetes.iotopology-mode`
