...
with [volumes](docsconceptsstoragevolumes) is suggested, in
particular PersistentVolumeClaim and PersistentVolume.

Some applications need additional storage but dont care whether that
...
follow the Pods lifetime and get created and deleted along with the
...
different purposes
- [emptyDir](docsconceptsstoragevolumes#emptydir) empty at Pod startup,
...
- [configMap](docsconceptsstoragevolumes#configmap),
  ...
  [secret](docsconceptsstoragevolumes#secret) inject different
  kinds of Kubernetes data into a Pod
- [image](docsconceptsstoragevolumes#image) allows mounting container image files or artifacts,
  directly to a Pod.
- [CSI ephemeral volumes](#csi-ephemeral-volumes)
  similar to the previous volume kinds, but provided by special  drivers
  which specifically [support this feature](httpskubernetes-csi.github.iodocsephemeral-local-volumes.html)
...
[local ephemeral
storage](docsconceptsconfigurationmanage-resources-containers#local-ephemeral-storage).
...
The Kubernetes CSI [Drivers list](httpskubernetes-csi.github.iodocsdrivers.html)
...
aware Pod scheduling](docsconceptsstoragestorage-capacity) is *not*
...
Heres an example manifest for a Pod that uses CSI ephemeral storage
...
- Using an [admission webhook](docsreferenceaccess-authn-authzextensible-admission-controllers)
...
- Typical operations on volumes are supported assuming that the driver
  ...
  [snapshotting](docsconceptsstoragevolume-snapshots),
  [cloning](docsconceptsstoragevolume-pvc-datasource),
  [resizing](docsconceptsstoragepersistent-volumes#expanding-persistent-volumes-claims),
  and [storage capacity tracking](docsconceptsstoragestorage-capacity).
...
[parameters for a volume claim](docsreferencegeneratedkubernetes-api#ephemeralvolumesource-v1-core)
...
That triggers volume binding andor provisioning, either immediately if
the  uses immediate volume binding or when the Pod is
...
In terms of [resource ownership](docsconceptsarchitecturegarbage-collection#owners-dependents),
...
same namespace, so that these conflicts cant occur.
...
they should use an [admission webhook](docsreferenceaccess-authn-authzextensible-admission-controllers)
that rejects objects like Pods that have a generic ephemeral volume.

The normal [namespace quota for PVCs](docsconceptspolicyresource-quotas#storage-resource-quota)
...
See [local ephemeral storage](docsconceptsconfigurationmanage-resources-containers#local-ephemeral-storage).

# # # CSI ephemeral volumes

- For more information on the design, see the
  [Ephemeral Inline CSI volumes KEP](httpsgithub.comkubernetesenhancementsblobad6021b3d61a49040a3f835e12c8bb5424db2bbbkepssig-storage20190122-csi-inline-volumes.md).
- For more information on further development of this feature, see the
  [enhancement tracking issue #596](httpsgithub.comkubernetesenhancementsissues596).

# # # Generic ephemeral volumes

- For more information on the design, see the
  [Generic ephemeral inline volumes KEP](httpsgithub.comkubernetesenhancementsblobmasterkepssig-storage1698-generic-ephemeral-volumesREADME.md).
