...
with [volume snapshots](docsconceptsstoragevolume-snapshots) and
[storage classes](docsconceptsstoragestorage-classes) is suggested.
...
that dont request any particular class to bind to by adding the
`snapshot.storage.kubernetes.iois-default-class true` annotation
...
metadata
  name csi-hostpath-snapclass
  annotations
    snapshot.storage.kubernetes.iois-default-class true
driver hostpath.csi.k8s.io
deletionPolicy Delete
parameters
```
...
Volume snapshot classes have a [deletionPolicy](docsconceptsstoragevolume-snapshots#delete).
