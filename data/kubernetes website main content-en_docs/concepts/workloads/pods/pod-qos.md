...
[resource requests](docsconceptsconfigurationmanage-resources-containers)
of the  in that Pod, along with
...
[Node Pressure](docsconceptsscheduling-evictionnode-pressure-eviction). The possible
...
[`static`](docstasksadminister-clustercpu-management-policies#static-policy) CPU management policy.
...
Pods in the `BestEffort` QoS class can use node resources that arent specifically assigned
...
A Pod has a QoS class of `BestEffort` if it doesnt meet the criteria for either `Guaranteed`
...
* If a Container exceeds its resource request and the node it runs on faces
  resource pressure, the Pod it is in becomes a candidate for [eviction](docsconceptsscheduling-evictionnode-pressure-eviction).
...
* The kube-scheduler does not consider QoS class when selecting which Pods to
  [preempt](docsconceptsscheduling-evictionpod-priority-preemption#preemption).
  Preemption can occur when a cluster does not have enough resources to run all the Pods
  you defined.

# #  heading whatsnext

* Learn about [resource management for Pods and Containers](docsconceptsconfigurationmanage-resources-containers).
* Learn about [Node-pressure eviction](docsconceptsscheduling-evictionnode-pressure-eviction).
* Learn about [Pod priority and preemption](docsconceptsscheduling-evictionpod-priority-preemption).
* Learn about [Pod disruptions](docsconceptsworkloadspodsdisruptions).
* Learn how to [assign memory resources to containers and pods](docstasksconfigure-pod-containerassign-memory-resource).
* Learn how to [assign CPU resources to containers and pods](docstasksconfigure-pod-containerassign-cpu-resource).
* Learn how to [configure Quality of Service for Pods](docstasksconfigure-pod-containerquality-service-pod).
