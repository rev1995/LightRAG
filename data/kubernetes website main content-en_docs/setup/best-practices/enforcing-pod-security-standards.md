...
The [Pod Security Admission Controller](docsreferenceaccess-authn-authzadmission-controllers#podsecurity)
...
each of them. Unlabeled namespaces should only indicate that theyve yet to be evaluated.

In the scenario that all workloads in all namespaces have the same security requirements,
we provide an [example](docstasksconfigure-pod-containerenforce-standards-namespace-labels#applying-to-all-namespaces)
...
within the desired level, enable the `warn` mode. If you expect to use audit logs to monitordrive
...
- [Kyverno](httpskyverno.iopolicies).
