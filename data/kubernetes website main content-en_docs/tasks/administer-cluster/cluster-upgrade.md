...
[install the latest version of `kubectl`](docstaskstools).
...
For each node in your cluster, [drain](docstasksadminister-clustersafely-drain-node)
...
# # # Switch your clusters storage API version

The objects that are serialized into etcd for a clusters internal
...
Refer to [API compatibility](docsconceptsextend-kubernetescompute-storage-netdevice-plugins#api-compatibility) and [Kubelet Device Manager API Versions](docsreferencenodedevice-plugin-api-versions) for more details.