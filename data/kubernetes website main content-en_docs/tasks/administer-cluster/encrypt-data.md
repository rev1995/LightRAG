...
* Your clusters control plane **must** use etcd v3.x (major version 3, any minor version).
  ...
  - resources
    ...
    providers
      - aescbc
          keys
          - name key2
            secret c2VjcmV0IGlzIHNlY3VyZSwgb3IgaXMgaXQCg
  - resources
...
since part of the configuration would be ineffective. The `resources` lists processing order and precedence
are determined by the order its listed in the configuration.
...
[encryption configuration API](docsreferenceconfig-apiapiserver-config.v1).
...
Before you configure encryption-at-rest for data in your clusters Kubernetes API, you
...
Providers for Kubernetes encryption at rest
  ...
  Not recommended due to CBCs vulnerability to padding oracle attacks. Key material accessible from control plane host.
...
[Using a KMS provider for data encryption](docstasksadminister-clusterkms-provider).
...
```shell
head -c 32 devurandom  base64
```

You can use `devhwrng` instead of `devurandom` if you want to
use your PCs built-in hardware entropy source. Not all Linux
...
so that youre relying on KMS encryption.
...
1. Save the new encryption config file to `etckubernetesencenc.yaml` on the control-plane node.
1. Edit the manifest for the `kube-apiserver` static pod `etckubernetesmanifestskube-apiserver.yaml` so that it is similar to
   ...
   metadata
     annotations
       kubeadm.kubernetes.iokube-apiserver.advertise-address.endpoint 10.20.30.40443
     creationTimestamp null
     labels
       app.kubernetes.iocomponent kube-apiserver
       tier control-plane
     name kube-apiserver
   ...
   spec
     ...
     - command
       - kube-apiserver
       ...
       - --encryption-provider-configetckubernetesencenc.yaml  # add this line
       volumeMounts
       ...
       - name enc                           # add this line
         mountPath etckubernetesenc      # add this line
     ...
     - name enc                             # add this line
       hostPath                             # add this line
         path etckubernetesenc           # add this line
...
nodes, this difference may mean that the kube-apiserver cant decrypt data.
...
1. Using the `etcdctl` command line tool, read that Secret out of etcd
   ...
   ```
   ETCDCTL_API3 etcdctl get registrysecretsdefaultsecret1 [...]  hexdump -C
   ```
   ...
   ETCDCTL_API3 etcdctl
      ...
      get registrysecretsdefaultsecret1  hexdump -C
   ```

   The output is similar to this (abbreviated)

   ```hexdump
   00000000  2f 72 65 67 69 73 74 72  79 2f 73 65 63 72 65 74  registrysecret
   00000010  73 2f 64 65 66 61 75 6c  74 2f 73 65 63 72 65 74  sdefaultsecret
   00000020  31 0a 6b 38 73 3a 65 6e  63 3a 61 65 73 63 62 63  1.k8sencaescbc
   00000030  3a 76 31 3a 6b 65 79 31  3a c7 6c e7 d3 09 bc 06  v1key1.l.....
...
1. Verify the stored Secret is prefixed with `k8sencaescbcv1` which indicates
...
1. Verify the Secret is correctly decrypted when retrieved via the API
   ...
   read
   [decoding a Secret](docstasksconfigmap-secretmanaging-secret-using-kubectl#decoding-secret)
   to learn how to completely decode the Secret.

# # # Ensure all relevant data are encrypted #ensure-all-secrets-are-encrypted

Its often not enough to make sure that new objects get encrypted you also want that
...
encryption, you can remove the API servers ability to read that APIs backing data
as plaintext.
...
dont use encryption, you can remove the encryption settings from the
`kube-apiserver`.
...
`--encryption-provider-config-automatic-reloadtrue`.
...
* Read about [decrypting data that are already stored at rest](docstasksadminister-clusterdecrypt-data)
* Learn more about the [EncryptionConfiguration configuration API (v1)](docsreferenceconfig-apiapiserver-config.v1).
