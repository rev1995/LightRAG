...
This means that the pod will get scheduled only on a node that has a `disktypessd` label.

 code_sample filepodspod-nginx-required-affinity.yaml

1. Apply the manifest to create a Pod that is scheduled onto your
   chosen node

    ```shell
    kubectl apply -f httpsk8s.ioexamplespodspod-nginx-required-affinity.yaml
    ```
...
This means that the pod will prefer a node that has a `disktypessd` label.

 code_sample filepodspod-nginx-preferred-affinity.yaml

1. Apply the manifest to create a Pod that is scheduled onto your
   chosen node

    ```shell
    kubectl apply -f httpsk8s.ioexamplespodspod-nginx-preferred-affinity.yaml
    ```
