...
[compatibility requirements](httpsdocs.microsoft.comvirtualizationwindowscontainersdeploy-containersversion-compatibility)
...
HostProcess containers have access to the hosts network interfaces and IP addresses.
...
[How to install containerd.](docssetupproduction-environmentcontainer-runtimes#containerd)
...
configuration. Of the policies defined in the [Pod Security Standards](docsconceptssecuritypod-security-standards)
...
the configurations which need to be set to enable the creation of a HostProcess pod
  ...
  Privileged policy specification
      ...
      hostNetwork
        ...
        Pods container HostProcess containers must use the hosts network namespace.
        Allowed Values
...
Applications running inside the container can access volume mounts directly via the volumeMounts
...
- `cvarrunsecretskubernetes.ioserviceaccount`
- `varrunsecretskubernetes.ioserviceaccount`
- `CONTAINER_SANDBOX_MOUNT_POINTvarrunsecretskubernetes.ioserviceaccount`
...
- **[LocalSystem](httpsdocs.microsoft.comwindowswin32serviceslocalsystem-account)**
- **[LocalService](httpsdocs.microsoft.comwindowswin32serviceslocalservice-account)**
- **[NetworkService](httpsdocs.microsoft.comwindowswin32servicesnetworkservice-account)**
...
1. Grant access to desired resources on the node to the local usergroup.
   This can be done with tools like [icacls](httpslearn.microsoft.comen-uswindows-serveradministrationwindows-commandsicacls).
...
HostProcess containers can be built from any of the existing [Windows Container base images](httpslearn.microsoft.comvirtualizationwindowscontainersmanage-containerscontainer-base-images).

Additionally a new base mage has been created just for HostProcess containers!
For more information please check out the [windows-host-process-containers-base-image github project](httpsgithub.commicrosoftwindows-host-process-containers-base-image#overview).
