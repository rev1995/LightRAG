...
For that you should check out [this guide](docstasksdebugdebug-cluster).
...
* **You dont have enough resources** You may have exhausted the supply of CPU
  or Memory in your cluster, in this case you need to delete Pods, adjust resource
  requests, or add new nodes to your cluster. See [Compute Resources document](docsconceptsconfigurationmanage-resources-containers)
...
but it cant run on that machine. Again, the information from `kubectl describe ...`
...
This typically happens if the Pod has a [finalizer](docsconceptsoverviewworking-with-objectsfinalizers)
and there is an [admission webhook](docsreferenceaccess-authn-authzextensible-admission-controllers)
...
[Debug Running Pods](docstasksdebugdebug-applicationdebug-running-pod)
...
```shell
I0805 104325.129850   46757 schema.go126] unknown field commnd
I0805 104325.129973   46757 schema.go129] this may be a false alarm, see httpsgithub.comkuberneteskubernetesissues6842
podsmypod
```
...
For example, run `kubectl get podsmypod -o yaml  mypod-on-apiserver.yaml` and then
...
Replication controllers are fairly straightforward. They can either create Pods or they cant.
If they cant create pods, then please refer to the
...
```shell
kubectl get endpointslices -l kubernetes.ioservice-nameSERVICE_NAME
```
...
IP addresses in the Services endpoint slices.
...
```shell
kubectl get pods --selectornamenginx,typefrontend
```

to list pods that match this selector. Verify that the list matches the Pods that you expect to provide your Service.
Verify that the pods `containerPort` matches up with the Services `targetPort`

# # # # Network traffic is not forwarded

Please see [debugging service](docstasksdebugdebug-applicationdebug-service) for more information.

# #  heading whatsnext

If none of the above solves your problem, follow the instructions in
[Debugging Service document](docstasksdebugdebug-applicationdebug-service)
...
You may also visit [troubleshooting document](docstasksdebug) for more information.
