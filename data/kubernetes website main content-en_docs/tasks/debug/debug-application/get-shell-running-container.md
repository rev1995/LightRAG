...
runs the nginx image. Here is the configuration file for the Pod
 ...
 code_sample fileapplicationshell-demo.yaml

Create the Pod

```shell
kubectl apply -f httpsk8s.ioexamplesapplicationshell-demo.yaml
```
...
```shell
kubectl exec --stdin --tty shell-demo -- binbash
```
...
at `usrsharenginxhtml`.

In your shell, create an `index.html` file in the `usrsharenginxhtml`
directory

```shell
# Run this inside the container
echo Hello shell demo  usrsharenginxhtmlindex.html
```
...
kubectl exec shell-demo -- cat proc1mounts
```
...
```shell
kubectl exec -i -t my-pod --container main-app -- binbash
```
...
* Read about [kubectl exec](docsreferencegeneratedkubectlkubectl-commands#exec)
