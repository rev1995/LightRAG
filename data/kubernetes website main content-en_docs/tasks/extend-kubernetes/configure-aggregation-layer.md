...
Configuring the [aggregation layer](docsconceptsextend-kubernetesapi-extensionapiserver-aggregation)
...
Reusing the same CA for different client types can negatively impact the clusters
...
![aggregation auth flows](imagesdocsaggregation-api-auth-flow.png)
...
4. The aggregator opens a connection to the aggregated API server using
   `--proxy-client-cert-file``--proxy-client-key-file` client certificatekey to secure the channel
...
6. The aggregated apiserver authenticates the incoming request using the auth proxy authentication method
  ...
  * pulls user info from the incoming requests http headers
...
[Authenticating to a Cluster](docsreferenceaccess-authn-authzauthentication).
...
the above. Many do it by default, leveraging the `k8s.ioapiserver` package.
...
The extension apiserver now can validate that the usergroup retrieved from
the headers are authorized to execute the given request. It does so by sending
a standard [SubjectAccessReview](docsreferenceaccess-authn-authzauthorization)
...
Kubernetes includes a default `ClusterRole` named `systemauth-delegator` that
has the appropriate permissions. It can be granted to the extension apiservers service account.
...
They may have already been taken care of by your provider.
    ...
    --requestheader-allowed-namesfront-proxy-client
...
* `--client-ca-file` When a request arrives to the Kubernetes apiserver,
  ...
  See the [documentation on TLS authentication](docsreferenceaccess-authn-authzauthentication#x509-client-certificates).
...
the risks and the mechanisms to protect the CAs usage.
...
apiVersion apiregistration.k8s.iov1
kind APIService
metadata
  name
spec
...
[path segment name](docsconceptsoverviewworking-with-objectsnames#path-segment-names).
...
* [Set up an extension api-server](docstasksextend-kubernetessetup-extension-api-server)
  to work with the aggregation layer.
* For a high level overview, see
  [Extending the Kubernetes API with the aggregation layer](docsconceptsextend-kubernetesapi-extensionapiserver-aggregation).
