...
[custom resource](docsconceptsextend-kubernetesapi-extensioncustom-resources)
into the Kubernetes API by creating a
[CustomResourceDefinition](docsreferencegeneratedkubernetes-api#customresourcedefinition-v1-apiextensions-k8s-io).
...
as specified in the CRDs `spec.scope` field. As with existing built-in
...
spec
  ...
  versions
    - name v1
      # Each version can be enableddisabled by Served flag.
      served true
...
```
apisstable.example.comv1namespaces*crontabs...
```
...
```yaml
apiVersion stable.example.comv1
kind CronTab
metadata
  ...
  metadata
    annotations
      kubectl.kubernetes.iolast-applied-configuration
        apiVersionstable.example.comv1,kindCronTab,metadataannotations,namemy-new-cron-object,namespacedefault,speccronSpec* * * * *5,imagemy-awesome-cron-image
    creationTimestamp 2021-06-20T073527Z
    generation 1
    name my-new-cron-object
    namespace default
...
With `apiextensions.k8s.iov1` the definition of a structural schema is
...
2. for each field in an object and each item in an array which is specified within any of `allOf`, `anyOf`,
   `oneOf` or `not`, the schema also specifies the fielditem outside of those logical junctors (compare example 1 and 2).
...
CustomResourceDefinitions store validated resource data in the clusters persistence store, .
As with native Kubernetes resources such as ,
...
CRDs converted from `apiextensions.k8s.iov1beta1` to `apiextensions.k8s.iov1` might lack
structural schemas, and `spec.preserveUnknownFields` might be `true`.

For legacy CustomResourceDefinition objects created as
`apiextensions.k8s.iov1beta1` with `spec.preserveUnknownFields` set to
`true`, the following is also true

* Pruning is not enabled.
* You can store arbitrary data.

For compatibility with `apiextensions.k8s.iov1`, update your custom
...
```shell
kubectl create --validatefalse -f my-crontab.yaml -o yaml
```
...
kind CronTab
metadata
  creationTimestamp 2017-05-31T125635Z
  generation 1
  name my-new-cron-object
  namespace default
  resourceVersion 285
...
This example turned off client-side validation to demonstrate the API servers behavior, by adding
the `--validatefalse` command line option.
...
RawExtensions (as in [`runtime.RawExtension`](docsreferencekubernetes-apiworkload-resourcescontroller-revision-v1#RawExtension))
...
See [Custom resource definition versioning](docstasksextend-kubernetescustom-resourcescustom-resource-definition-versioning)
...
metadata
  finalizers
  - stable.example.comfinalizer
```

Identifiers of custom finalizers consist of a domain name, a forward slash and the name of
the finalizer. Any controller can add a finalizer to any objects list of finalizers.
...
[OpenAPI v3.0 schemas](httpsgithub.comOAIOpenAPI-Specificationblob3.0.0versions3.0.0.md#schema-object),
...
[admission webhooks](docsreferenceaccess-authn-authzadmission-controllers#validatingadmissionwebhook).
...
[Common Expression Language (CEL)](httpsgithub.comgooglecel-spec) expressions when the
...
```console
The CronTab my-new-cron-object is invalid [] Invalid value map[string]interface apiVersionstable.example.comv1, kindCronTab, metadatamap[string]interface namemy-new-cron-object, namespacedefault, deletionTimestampinterface (nil), deletionGracePeriodSeconds(*int64)(nil), creationTimestamp2017-09-05T052007Z, uide14d79e7-91f9-11e7-a598-f0761cb232d1, clusterName, specmap[string]interface cronSpec* * * *, imagemy-awesome-cron-image, replicas15
...
[feature gate](docsreferencecommand-line-tools-referencefeature-gates) to
...
- `properties`
  Addingremovingmodifying the names of properties is not ratcheted, but
...
- `additionalProperties`
  ...
  ratcheted.
- `metadata`
  Errors that come from Kubernetes built-in validation of an objects `metadata`
...
Validation rules use the [Common Expression Language (CEL)](httpsgithub.comgooglecel-spec)
  ...
  openAPIV3Schema
    ...
    properties
      spec
        type object
        x-kubernetes-validations
          - rule self.minReplicas
You can quickly test CEL expressions in [CEL Playground](httpsplaycel.undistro.io).

Validation rules are compiled when CRDs are createdupdated.
The request of CRDs createupdate will fail if compilation of validation rules fail.
...
- `no_matching_overload` this function has no overload for the types of the arguments.
  ...
  ```none
  Invalid value apiextensions.ValidationRuleRuleself  true, Message compilation failed ERROR 16 found no matching overload for __ applied to (int, bool)
  ```
...
- `no_such_field` does not contain the desired field.
  ...
  ```none
  Invalid value apiextensions.ValidationRuleRuleself.nonExistingField  0, Message compilation failed ERROR 15 undefined field nonExistingField
  ```
...
- `invalid argument` invalid argument to macros.
  ...
  ```none
  Invalid value apiextensions.ValidationRuleRulehas(self), Message compilation failed ERROR 14 invalid argument to has() macro
  ```
...
Validation Rules Examples
 ...
 Rule                                                                              Purpose
 ----------------                                                                  ------------
 `self.minReplicas !(MY_KEY in self.map1) #124#124 self[MY_KEY].matches([a-zA-Z]*)                Validate the value of a map for a specific key, if it is in the map
...
type of the field rule scoped to     Rule example
 ----------------------- -----------------------
 root object             `self.status.actual  0  value  0`
...
Here is the declarations type mapping between OpenAPIv3 and CEL type
 ...
 array                                              list
 array with x-kubernetes-list-typemap             list with map based Equality  unique key guarantees
 array with x-kubernetes-list-typeset             list with set based Equality  unique entry guarantees
 boolean                                           boolean
 number (all formats)                              double
 integer (all formats)                             int (64)
 null                                              null_type
 string                                            string
 string with formatbyte (base64 encoded)          bytes
 string with formatdate                           timestamp (google.protobuf.Timestamp)
 string with formatdatetime                       timestamp (google.protobuf.Timestamp)
 string with formatduration                       duration (google.protobuf.Duration)

xref [CEL types](httpsgithub.comgooglecel-specblobv0.6.0doclangdef.md#values),
[OpenAPI types](httpsswagger.iospecification#data-types),
[Kubernetes Structural Schemas](docstasksextend-kubernetescustom-resourcescustom-resource-definitions#specifying-a-structural-schema).
...
```yaml
x-kubernetes-validations
- rule self.x )` function to cast the scalar to a string
...
the CustomResourceDefinition API doesnt include this field, and trying to set it may result
in an error.
...
evaluated and the type of `oldSelf` be changed to a CEL [`Optional`](httpspkg.go.devgithub.comgooglecel-gocel#OptionalTypes) type.
...
Example Usage
 ...
 CEL                                      Description
------------------------------------------------------
 self.foo  foo #124#124 (oldSelf.hasValue()  oldSelf.value().foo ! foo)  Ratcheted rule. Once a value is set to foo, it must stay foo. But if it existed before the foo constraint was introduced, it may use any value
 [oldSelf.orValue(), self].all(x, [OldCase1, OldCase2].exists(case, x  case)) #124#124 [NewCase1, NewCase2].exists(case, self  case) #124#124 [NewCase].has(self)  Ratcheted validation for removed enum cases if oldSelf used them
 oldSelf.optMap(o, o.size()).orValue(0)  4  Ratcheted validation of newly increased minimum map or list size

# # # # Validation functions #available-validation-functions

Functions available include

- CEL standard functions, defined in the [list of standard definitions](httpsgithub.comgooglecel-specblobv0.7.0doclangdef.md#list-of-standard-definitions)
- CEL standard [macros](httpsgithub.comgooglecel-specblobv0.7.0doclangdef.md#macros)
- CEL [extended string function library](httpspkg.go.devgithub.comgooglecel-gov0.11.2ext#Strings)
- Kubernetes [CEL extension library](httpspkg.go.devk8s.ioapiextensions-apiserverv0.24.0pkgapiserverschemacellibrary#pkg-functions)
...
- The path to the schema node being validated by a transition rule must resolve to a node that is
  ...
  descendants (`spec.foo[10].bar`) cant necessarily be correlated between an existing object and a
...
A portion of the schema is correlatable if all `array` parent schemas are of type `x-kubernetes-list-typemap`
...
Here are some examples for transition rules
 ...
 Prevent modificationremoval once assigned                         `oldSelf ! bar  self  bar` or `!has(oldSelf.field)  has(self.field)`
...
To use defaulting, your CustomResourceDefinition must use API version `apiextensions.k8s.iov1`.
...
Null values for fields that either dont specify the nullable flag, or give it a
`false` value, will be pruned before defaulting happens. If a default is present, it will be
applied. When nullable is `true`, null values will be conserved and wont be defaulted.
...
as [OpenAPI v3](docsconceptsoverviewkubernetes-api#openapi-and-swagger-definitions) and
...
The [kubectl](docsreferencekubectl) command-line tool consumes the published schema to perform
...
[OpenAPI v2 spec](docsconceptsoverviewkubernetes-api#openapi-and-swagger-definitions).
...
valid OpenAPI schemas that it doesnt understand. The conversion wont modify the validation schema defined in CRD,
and therefore wont affect [validation](#validation) in the API server.

1. The following fields are removed as they arent supported by OpenAPI v2.
...
The kubectl tool relies on server-side output formatting. Your clusters API server decides which
...
A columns `type` field can be any of the following (compare
[OpenAPI v3 data types](httpsgithub.comOAIOpenAPI-Specificationblobmasterversions3.0.0.md#dataTypes))
...
A columns `format` field can be any of the following
...
The columns `format` controls the style used when `kubectl` prints the value.

# # # Field selectors

[Field Selectors](docsconceptsoverviewworking-with-objectsfield-selectors)
...
The `spec.versions[*].selectableFields` field of a  may be used to
declare which other fields in a custom resource may be used in field selectors
with the feature of `CustomResourceFieldSelectors`
[feature gate](docsreferencecommand-line-tools-referencefeature-gates) (This feature gate is enabled by default since Kubernetes v1.31).
...
Save the CustomResourceDefinition to `shirt-resource-definition.yaml`
 ...
 code_sample filecustomresourcedefinitionshirt-resource-definition.yaml

Create the CustomResourceDefinition

```shell
kubectl apply -f httpsk8s.ioexamplescustomresourcedefinitionshirt-resource-definition.yaml
```

Define some Shirts by editing `shirt-resources.yaml` for example

 code_sample filecustomresourcedefinitionshirt-resources.yaml

Create the custom resources

```shell
kubectl apply -f httpsk8s.ioexamplescustomresourcedefinitionshirt-resources.yaml
```
...
```shell
kubectl get shirts.stable.example.com --field-selector spec.colorblue
```
...
```shell
kubectl get shirts.stable.example.com --field-selector spec.colorgreen,spec.sizeM
```
...
The `autoscalingv1.Scale` object is sent as the payload for `scale`.
...
```none
apisstable.example.comv1namespaces*crontabsstatus
```

and

```none
apisstable.example.comv1namespaces*crontabsscale
```
...
```shell
kubectl scale --replicas5 crontabsmy-new-cron-object
...
You can use a [PodDisruptionBudget](docstasksrun-applicationconfigure-pdb) to protect custom
...
```none
NAME                          AGE
crontabsmy-new-cron-object   3s
```

# #  heading whatsnext

* Read about [custom resources](docsconceptsextend-kubernetesapi-extensioncustom-resources).

* See [CustomResourceDefinition](docsreferencegeneratedkubernetes-api#customresourcedefinition-v1-apiextensions-k8s-io).

* Serve [multiple versions](docstasksextend-kubernetescustom-resourcescustom-resource-definition-versioning) of a
  CustomResourceDefinition.
