...
kubectl create -f httpskubernetes.ioexamplesapplicationjobrabbitmqrabbitmq-service.yaml
```
```
...
```shell
kubectl create -f httpskubernetes.ioexamplesapplicationjobrabbitmqrabbitmq-statefulset.yaml
```
```
...
kubectl run -i --tty temp --image ubuntu22.04
```
```
Waiting for pod defaulttemp-loe07 to be running, status is Pending, pod ready false
...
```
```
RABBITMQ_SERVICE_SERVICE_HOST10.0.147.152
```
...
export BROKER_URLamqpguestguestrabbitmq-service5672
# If you could not resolve rabbitmq-service in the previous step,
# then use this command instead
BROKER_URLamqpguestguestRABBITMQ_SERVICE_SERVICE_HOST5672

# Now create a queue

usrbinamqp-declare-queue --urlBROKER_URL -q foo -d
```
```
foo
```
...
```shell
usrbinamqp-publish --urlBROKER_URL -r foo -p -b Hello

# And get it back.

usrbinamqp-consume --urlBROKER_URL -q foo -c 1 cat  echo 12
```
```
Hello
```
...
usrbinamqp-declare-queue --urlBROKER_URL -q job1  -d
```
```
job1
```
...
do
  usrbinamqp-publish --urlBROKER_URL -r job1 -p -b f
done
```
...
example program
 ...
 code_sample languagepython fileapplicationjobrabbitmqworker.py
...
download the [Dockerfile](examplesapplicationjobrabbitmqDockerfile),
and [worker.py](examplesapplicationjobrabbitmqworker.py).  In either case,
...
```shell
docker tag job-wq-1 job-wq-1
docker push job-wq-1
```
...
Here is a manifest for a Job.  Youll need to make a copy of the Job manifest
(call it `.job.yaml`),
and edit the name of the container image to match the name you used.

 code_sample fileapplicationjobrabbitmqjob.yaml
...
kubectl wait --forconditioncomplete --timeout300s jobjob-wq-1
```

Next, check on the Job

```shell
kubectl describe jobsjob-wq-1
```
```
Name             job-wq-1
Namespace        default
Selector         controller-uid41d75705-92df-11e7-b85e-fa163ee3c11f
Labels           controller-uid41d75705-92df-11e7-b85e-fa163ee3c11f
                  job-namejob-wq-1
Annotations
Parallelism      2
Completions      8
Start Time       Wed, 06 Sep 2022 164202 0000
...
Pod Template
  ...
  Containers
   c
    Image      container-registry.examplecausal-jigsaw-637job-wq-1
    Port
    Environment
      BROKER_URL       amqpguestguestrabbitmq-service5672
      QUEUE            job1
    Mounts
  Volumes
Events
...
All the pods for that Job succeeded! Youre done.
...
want to consider one of the other [job patterns](docsconceptsworkloadscontrollersjob#job-patterns).

This approach creates a pod for every work item.  If your work items only take a few seconds,
though, creating a Pod for every work item may add a lot of overhead.  Consider another
design, such as in the [fine parallel work queue example](docstasksjobfine-parallel-processing-work-queue),
...
The [fine parallel work queue example](docstasksjobfine-parallel-processing-work-queue)
