...
The [StatefulSet controller](docsconceptsworkloadscontrollersstatefulset) is responsible for
...
[shuts down gracefully](docsconceptsworkloadspodspod-lifecycle#pod-termination)
...
[timeout](docsconceptsarchitecturenodes#condition).
...
- The Node object is deleted (either by you, or by the
  [Node Controller](docsconceptsarchitecturenodes#node-controller)).
...
```shell
kubectl delete pods  --grace-period0 --force
```

If youre using any version of kubectl  --grace-period0
```
...
```shell
kubectl patch pod  -p metadatafinalizersnull
```
