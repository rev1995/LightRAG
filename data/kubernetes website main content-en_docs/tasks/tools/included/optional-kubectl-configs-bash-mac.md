...
[**bash-completion**](httpsgithub.comscopbash-completion) which you thus have to previously install.
...
script **doesnt work** correctly with bash-completion v1 and Bash 3.2.
...
Bash 4.1 ([*instructions*](httpsapple.stackexchange.coma292760)).
The following instructions assume that you use Bash 4.1
(that is, any Bash version of 4.1 or newer).

# # # Upgrade Bash

The instructions here assume you use Bash 4.1. You can check your Bashs version by running

```bash
echo BASH_VERSION
```

If it is too old, you can installupgrade it using Homebrew
...
Homebrew usually installs it at `usrlocalbinbash`.
...
in which case kubectl completion wont work).
...
```bash
brew_etc(brew --prefix)etc  [[ -r brew_etcprofile.dbash_completion.sh ]]  . brew_etcprofile.dbash_completion.sh
```
...
- Add the completion script to the `usrlocaletcbash_completion.d` directory

    ```bash
    kubectl completion bash usrlocaletcbash_completion.dkubectl
    ```

- If you have an alias for kubectl, you can extend shell completion to work with that alias

    ```bash
    echo alias kkubectl .bash_profile
    echo complete -o default -F __start_kubectl k .bash_profile
    ```

- If you installed kubectl with Homebrew (as explained
  [here](docstaskstoolsinstall-kubectl-macos#install-with-homebrew-on-macos)),
  then the kubectl completion script should already be in `usrlocaletcbash_completion.dkubectl`.
  In that case, you dont need to do anything.

   The Homebrew installation of bash-completion v2 sources all the files in the
   `BASH_COMPLETION_COMPAT_DIR` directory, thats why the latter two methods work.
