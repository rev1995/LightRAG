...
In the following interactive tutorial, well update our application to a new version,
...
```shell
kubectl set image deploymentskubernetes-bootcamp kubernetes-bootcampdocker.iojocatalinkubernetes-bootcampv2
```
...
tutorial step, run `describe serviceskubernetes-bootcamp`. If its missing,
...
```shell
kubectl rollout status deploymentskubernetes-bootcamp
```
...
Lets perform another update, and try to deploy an image tagged with `v10`

```shell
kubectl set image deploymentskubernetes-bootcamp kubernetes-bootcampgcr.iogoogle-sampleskubernetes-bootcampv10
```
...
Notice that the output doesnt list the desired number of available Pods. Run the
...
```shell
kubectl rollout undo deploymentskubernetes-bootcamp
```
...
```shell
kubectl delete deploymentskubernetes-bootcamp serviceskubernetes-bootcamp
```
